...
REPLIED_FILE = os.path.join(DATA_DIR, "threads_replied.json")
...
# 최근 사용 답글 추적 (중복 방지)
RECENT_REPLIES_FILE = os.path.join(DATA_DIR, "threads_recent_replies.json")
...
async def auto_reply():
    ...
    async with async_playwright() as p:
        ...
        if _restricted:
            log("⛔ 계정 제한(restriction) 감지 — 자동화 차단됨. 24~48시간 대기 필요")
            await browser.close()
            return
