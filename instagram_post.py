...
async def post_carousel(img_paths: list, caption: str):
    ...
    async with async_playwright() as p:
        ...
        # 공유 버튼
        share_clicked = False
        # 1차: Playwright locator 기반
        for text in ["공유", "Share"]:
            try:
                btn = page.locator(f'button:has-text("{text}"), div[role="button"]:has-text("{text}"), span:has-text("{text}"), a:has-text("{text}")')
                if await btn.count() > 0:
                    await btn.first.click()
                    share_clicked = True
                    break
        ...
        if not share_clicked:
            share_clicked = await page.evaluate("""
                () => {
                    const els = document.querySelectorAll('div[role="button"], button, span, a, div[tabindex="0"]');
                    for (const el of els) {
            ...
            """)
        # 3차: 헤더 영역의 클릭 가능한 텍스트 탐색
        if not share_clicked:
            header_els = await page.query_selector_all('div[role="dialog"] div[role="button"], div[role="dialog"] button, div[role="dialog"] [tabindex]')
