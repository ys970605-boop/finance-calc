#!/usr/bin/env python3
"""
Chrome 네이버 쿠키 추출 -> Playwright storage state JSON 변환
macOS 전용 (Keychain + SQLite AES-CBC 복호화)

사용법:
  python3 extract_naver_cookies.py
  python3 extract_naver_cookies.py --profile "Default"
  python3 extract_naver_cookies.py --output my_session.json
  python3 extract_naver_cookies.py --verify  # 추출 후 Playwright로 로그인 검증
"""
...
def extract_naver_cookies(
    profile: str = None,
    domains: list = None,
    output_path: str = "naver_session.json",
) -> dict:
    """
    Chrome DB에서 네이버 쿠키를 추출해 Playwright storage state 형식으로 반환.

    Args:
        profile: Chrome 프로필 폴더명 (예: "Default", "Profile 1")
        domains: 추출할 도메인 목록 (기본: .naver.com)
    ...
    # 주요 쿠키 출력
    print("\n[주요 쿠키]")
    important = ["NID_AUT", "NID_SES", "NID_JKL", "NAVER_USERMETA", "nid_inf", "nid_slevel"]
    for cookie in playwright_cookies:
        if cookie["name"] in important:
            val_preview = cookie["value"][:40] + "..." if len(cookie["value"]) > 40 else cookie["value"]
            print(f"  {cookie['domain']:25s} | {cookie['name']:20s} = {val_preview}")
...
def verify_with_playwright(session_file: str):
    ...
    with sync_playwright() as p:
        ...
        # 로그인 여부 확인 (네이버 로그인 상태는 .naver.com의 NID_AUT 쿠키로 판별)
        cookies = ctx.cookies()
        nid_aut = next((c for c in cookies if c["name"] == "NID_AUT"), None)
        if nid_aut:
            print(f"[+] 로그인 성공! NID_AUT 쿠키 확인됨")
        else:
            print(f"[-] 로그인 실패. NID_AUT 쿠키 없음. URL: {page.url}")

        page.screenshot(path="/tmp/naver_verify.png")
        print("[+] 스크린샷: /tmp/naver_verify.png")
        browser.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Chrome 네이버 쿠키 -> Playwright session JSON 변환"
    )
    parser.add_argument(
        "--profile",
        default=None,
    ...
    parser.add_argument(
        "--output",
        default="naver_session.json",
        help="출력 JSON 파일 경로 (기본: naver_session.json)",
    )
    parser.add_argument(
        "--domains",
        nargs="+",
        default=None,
        help="추출할 도메인 LIKE 패턴 (기본: %%naver.com%%)",
    )
    parser.add_argument(
        "--verify",
        action="store_true",
        help="추출 후 Playwright로 네이버 로그인 검증",
    )
    parser.add_argument(
        "--list-profiles",
        action="store_true",
    ...
    try:
        storage_state = extract_naver_cookies(
