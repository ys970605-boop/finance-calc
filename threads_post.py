...
POSTED_FILE = os.path.join(DATA_DIR, "threads_posted.json")
...
POSTS = [
    ...
    (
        ...
        "▶ 주휴수당 계산기 → calcmoney.kr/weekly-holiday"
    ),
    (
        "💼 10년 다니면 퇴직금 얼마나 될까?\n\n월급 300만원 × 10년 = 3000만원\n근데 IRP로 받으면 세금 아낄 수 있어\n\n퇴직금 얼마 모았어? 댓글에 공유해봐 👇\n\n#재테크",
        "▶ 퇴직금 계산기 → calcmoney.kr/severance"
    ),
    (
    ...
    (
        ...
        "▶ 청약 가점 계산기 → calcmoney.kr/subscription"
    ),
    (
        "💸 집 팔 때 양도세 얼마 나오는지 알아?\n\n2년 보유 + 2년 거주하면 12억까지 비과세\n조건 안 맞으면 수천만원 세금 폭탄\n\n이거 몰랐던 사람 있어? 솔직히 말해봐 👇\n\n#부동산",
        "▶ 양도소득세 계산기 → calcmoney.kr/capital-gains"
    ),
    (
...
async def post_thread(body, comment):
    ...
    async with async_playwright() as p:
        ...
        # restriction 감지용 플래그
        _restricted = False
        def _check_restriction(resp):
            nonlocal _restricted
            if resp.status in (301, 302) and "restriction" in (resp.headers.get("location", "")):
                _restricted = True
        page.on("response", _check_restriction)

        # graphql __user=0 → 실제 사용자 ID 교체 (스팸 차단 방지)
        async def fix_user_route(route):
            request = route.request
            if "graphql" in request.url:
                post_data = request.post_data or ""
                if "__user=0" in post_data:
                    post_data = post_data.replace("__user=0", f"__user={user_id}")
                await route.continue_(post_data=post_data)
            else:
                await route.continue_()

        await page.route("**/*", fix_user_route)
        ...
        # 스플래시/오버레이 제거 (헤드리스 환경에서 클릭 차단 방지)
        await page.evaluate("""
            document.getElementById('barcelona-splash-screen')?.remove();
            document.querySelectorAll('[data-visualcompletion="ignore"]').forEach(el => {
                if (el.style && getComputedStyle(el).pointerEvents !== 'none') el.remove();
            });
        """)

        # restriction 감지 시 즉시 중단
        if _restricted:
            print(f"[{datetime.now()}] ⛔ 계정 제한(restriction) 감지 — 자동화 일시 중단 필요")
            log("⛔ 계정 제한(restriction) 감지 — Threads에서 자동화 차단됨. 24~48시간 대기 후 재시도 필요")
            await browser.close()
            return False

        # 입력창 클릭 - "새로운 소식이 있나요?" / "What's new?" placeholder
        clicked = False
        ...
        if not clicked:
            # fallback: "만들기"/"Create" 네비 버튼
            btns = await page.query_selector_all("div[role='button'], button, a")
            for btn in btns:
                txt = await btn.text_content()
                if txt and txt.strip() in ("만들기", "Create"):
                    is_visible = await btn.is_visible()
                    bb = await btn.bounding_box()
                    if is_visible and bb and bb['x'] < 150:
                        await btn.click()
                        clicked = True
                        break
...
def log(msg):
    with open(LOG_FILE, "a", encoding="utf-8") as f:
        f.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [게시] {msg}\n")
...
def main():
    ...
    if not os.path.exists(SESSION_FILE):
        log("⛔ 세션 파일 없음 — python3 threads_login.py 실행 필요")
        return
